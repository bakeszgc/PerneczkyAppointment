<div {{ $attributes->merge(['class' => 'relative rounded-md overflow-hidden shadow-xl cursor-pointer group']) }} >
    <img src="{{ $barber == 'earliest' ? asset('pictures/pfp_blank.png') : $barber->getPicture() }}" alt="{{ $barber == 'earliest' ? 'Earliest' : $barber->getName() }}" class=" min-w-full rounded-md z-0 transition-all group-hover:scale-105">
    ...
    <h2 class="font-black text-xl max-lg:text-lg max-md:text-base max-sm:text-sm absolute bottom-[5%] left-[5%] text-white z-20 text-left">
        {{ $barber == 'earliest' ? 'Earliest available' : $barber->getName() }}
    </h2>
</div>