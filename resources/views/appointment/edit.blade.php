...
<x-user-layout :title="$title" currentView="{{ $access }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            appStartInputs.forEach(input => {
                input.addEventListener('change', function () {
                    ...
                    renderTimeOff (calendar, appStartDate, appStartHour, appStartMinute, appEndDate, appEndHour, appEndMinute, barberInput, appointments);
                });
            });
            ...
            renderAppointments(appointments, barberInput, date, calendar);
            renderTimeOff (calendar, appStartDate, appStartHour, appStartMinute, appEndDate, appEndHour, appEndMinute, barberInput, appointments);
        ...
        function renderAppointments (appointments, barberInput, date, calendar) {
            ...
            const filtered = appointments.filter(app => {
                ...
                return (
                    app.barber_id == barberInput.value &&
                    appStart >= start &&
                    appStart < end
                );
            });
            ...
            filtered.forEach(app => {
                ...
                const duration = (appEndTime - appStartTime) / 1000 / 60;

                const hour = appStartTime.getHours();
                const minute = (appStartTime.getMinutes() == 0) ? '00' : appStartTime.getMinutes();
                const customerName = app.user.first_name;

                // CREATING THE MAIN DIV ELEMENT
                const div = document.createElement('div');
                div.classList.add('absolute','w-1/8','p-0.5','appointment');
                div.style.top = 53/60 * (hour * 60 + parseInt(minute)) - 486 + 'px';
                div.style.left = (appStartTime.getDay() == 0 ? 7 : appStartTime.getDay()) * 12.5 + '%';
                div.style.height = duration / 60 * 53 + 'px';

                // CREATING THE A ELEMENT
                const link = document.createElement('a');
                link.href = ((app.service_id == 1) ? "/admin/time-offs/" : "/admin/bookings/") + app.id;
                
                // CREATING THE INNER DIV ELEMENT
                const innerDiv = document.createElement('div');
                innerDiv.classList.add('border','border-l-4','transition-all','h-full','px-1','max-sm:px-0.5','rounded-md','max-lg:translate-y-6','overflow-hidden','max-sm:text-xs');
                if (appStartTime < new Date()) innerDiv.classList.add('bg-slate-100','hover:bg-slate-200','text-slate-600','border-slate-300');
                if (appStartTime >= new Date() && app.service_id == 1) innerDiv.classList.add('bg-green-100','hover:bg-green-200','border-green-400','text-green-600');
                if (appStartTime >= new Date() && app.service_id != 1) innerDiv.classList.add('bg-blue-100','hover:bg-blue-200','border-blue-300','text-blue-600');
                ...
                if (duration >= 30) {
                    ...
                    spanName.innerHTML = (app.service_id == 1) ? 'TIME OFF' : customerName;
        ...
        function renderTimeOff (calendar, appStartDate, appStartHour, appStartMinute, appEndDate, appEndHour, appEndMinute, barberInput, appointments) {
            ...
            // REMOVING EXISTING NEW TIME OFF DIV ELEMENT
            document.querySelectorAll('.newTimeOff').forEach(el => el.remove());

            // CHECKING THE DAYSPAN OF THE SELECTED TIME OFF
            const dayDifference = (new Date(appEndDate.value) - new Date(appStartDate.value)) / 1000 / 60 / 60 / 24;
            for (let index = 0; index < dayDifference + 1; index++) {

                // DECLARING VARIABLES FOR EACH DIV ELEMENTS
                const start = (index == 0) ? appStartTime : addDays(new Date(appStartDate.value + ' 10:00'),index);
                const end = (index == dayDifference) ? appEndTime : addDays(new Date(appStartDate.value + ' 20:00'),index);
                ...
                if (start >= getFirstDayOfWeek(appStartTime) && start < addDays(getFirstDayOfWeek(appStartTime),7)) {
                    ...
                    const div = document.createElement('div');
                    div.classList.add('newTimeOff','absolute','w-1/8','p-0.5','z-10');
                    ...
                    // CREATING THE INNER DIV ELEMENT
                    const innerDiv = document.createElement('div');
                    innerDiv.classList.add('border','border-dashed','transition-all','h-full','px-1','max-sm:px-0.5','rounded-md','max-lg:translate-y-6','overflow-hidden','max-sm:text-xs','bg-green-100','hover:bg-green-200','border-green-400','text-green-600');

                    // CHECKING IF THERE ARE CLASHES WITH OTHER APPOINTMENTS
                    let count = 0;
                    if (barberInput.value != 'empty') {
                        ...
                        count = startingDuring.length + endingDuring.length + startingBeforeEndingAfter.length;

                        if (count != 0) {
                            innerDiv.classList.remove('bg-green-100','hover:bg-green-200','border-green-400','text-green-600');
                    ...
                    if (duration >= 30) {
                        ...
                        const spanName = document.createElement('span');
                        spanName.classList.add('font-normal');
                        spanName.innerHTML = (count == 0) ? 'NEW TIME OFF' : 'OVERLAPPING';                
